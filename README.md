...
// PRECACHE vide intentionnellement : on ne pré-cache rien à l'install
// pour éviter les échecs réseau qui bloqueraient l'activation.
// Le cache se construit au fil des requêtes (runtime caching).
const PRECACHE = [];

self.addEventListener('install', event => {
  // Activation immédiate sans attendre de pré-cache
  self.skipWaiting();
});
