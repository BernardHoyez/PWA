            ...
            if (editWaypointsCheckbox.checked) {
                processedImages.forEach(img => {
                    ...
                    if (customName) {
                        ...
                        // Find the placemark containing this image and replace its name
                        const placemarkPattern = new RegExp(
                            `(<Placemark[^>]*>[\\s\\S]*?<href>${githubUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}</href>[\\s\\S]*?<name>)([^<]*)(</name>)`,
                            'i'
                        );
                        
                        modifiedKml = modifiedKml.replace(placemarkPattern, `$1${customName}$3`);
                    }
                });
            }
            
            // Add "View full size" link in descriptions
            if (addImageLinksCheckbox.checked) {
                processedImages.forEach(img => {
                    const githubUrl = githubBase + img.name;
                    const escapedUrl = githubUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
                    
                    // Find description tag for this specific placemark
                    const placemarkPattern = new RegExp(
                        `(<Placemark[^>]*>[\\s\\S]*?<href>${escapedUrl}</href>[\\s\\S]*?<description>)([\\s\\S]*?)(</description>)`,
                        'i'
                    );
                    
                    modifiedKml = modifiedKml.replace(placemarkPattern, (match, p1, p2, p3) => {
                        // Add link if not already present
                        if (!p2.includes('Agrandir l\'image')) {
                            const linkHTML = `<![CDATA[<br/><br/><a href="${githubUrl}" target="_blank" style="display:inline-block;padding:8px 16px;background:#2563eb;color:white;text-decoration:none;border-radius:6px;font-weight:600;">🔍 Agrandir l'image</a>]]>`;
                            return p1 + p2 + linkHTML + p3;
                        }
                        return match;
                    });
                });
