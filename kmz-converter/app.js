...
async function downloadModifiedKmz() {
    ...
    try {
        ...
        // For each image, update the Placemark
        processedImages.forEach(img => {
            ...
            // STEP 1: Find the Placemark containing this image
            // Then find the <n><![CDATA[...]]></n> within that Placemark
            
            // First, escape the filename for regex
            const escapedPath = localPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
            
            // Find the entire Placemark block containing this image
            ...
            if (placemarkMatch) {
                console.log(`  ✓ Found Placemark for this image`);
                
                const placemarkContent = placemarkMatch[0];
                
                // Within this Placemark, replace the name
                const nameRegex = /(<n><!\[CDATA\[)([^\]]*)(]\]><\/n>)/i;
                const nameMatch = placemarkContent.match(nameRegex);
                
                if (nameMatch) {
                    console.log(`  ✓ Found name: "${nameMatch[2]}"`);
                    
                    const updatedPlacemark = placemarkContent.replace(
                        nameRegex,
                        `$1${data.name}$3`
                    );
                    
                    modifiedKml = modifiedKml.replace(placemarkContent, updatedPlacemark);
                    console.log(`  ✓ Name replaced with: "${data.name}"`);
                } else {
                    console.log(`  ✗ Could not find <n><![CDATA[...]]></n> pattern`);
                    console.log(`  Placemark content:`, placemarkContent.substring(0, 200));
                }
            } else {
                console.log(`  ✗ Could not find Placemark containing this image`);
            }
            
            // Add comment to description if provided
            if (data.comment) {
                // OruxMaps structure: <description><![CDATA[<table...>...]]></description>
                const descPattern = new RegExp(
                    `(<Placemark>[\\s\\S]*?<description><!\\[CDATA\\[)(<table[^>]*>[\\s\\S]*?<img[^>]*src="${escapedPath}"[^>]*>[\\s\\S]*?</table>)`,
                    'i'
                );
                
                const descMatch = modifiedKml.match(descPattern);
                if (descMatch) {
                    console.log(`  ✓ Description pattern matched, adding comment`);
                    modifiedKml = modifiedKml.replace(descPattern, 
                        `$1<p style="font-weight:600;margin-bottom:10px;color:#2563eb;">${data.comment}</p>$2`);
                } else {
                    console.log(`  ✗ Description pattern NOT matched`);
                }
            }
        });
    ...
    try {
        ...
        processedImages.forEach(img => {
            ...
            const placemarkPattern = new RegExp(
                `(<Placemark>[\\s\\S]*?<n><!\\[CDATA\\[)([^\\]]*)(\\]\\]></n>[\\s\\S]*?<img[^>]*src="${localPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>[\\s\\S]*?</Placemark>)`,
                'i'
            );
            
            finalKml = finalKml.replace(placemarkPattern, `$1${data.name}$3`);
            
            // Add comment to description
            if (data.comment) {
                const descPattern = new RegExp(
                    `(<Placemark>[\\s\\S]*?<description><!\\[CDATA\\[)(<table[^>]*>[\\s\\S]*?<img[^>]*src="${localPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>[\\s\\S]*?</table>)`,
                    'i'
                );
