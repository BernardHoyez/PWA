...
async function downloadModifiedKmz() {
    ...
    try {
        ...
        processedImages.forEach(img => {
            ...
            console.log(`  Searching for pattern: files/${escapedName}`);
            
            const match = modifiedKml.match(placemarkPattern);
            if (!match) {
                console.log('  ✗ Placemark NOT found');
                console.log(`  DEBUG: Searching in KML length: ${modifiedKml.length}`);
                console.log(`  DEBUG: Does KML contain "files/${img.name}"?`, modifiedKml.includes(`files/${img.name}`));
                return;
            }
            
            console.log('  ✓ Found Placemark - length:', match[1].length);
            
            let placemark = match[1];
            console.log('  ✓ Found Placemark');
            
            // Show a preview of what was found
            const preview = placemark.substring(0, 200).replace(/\n/g, ' ');
            ...
            // DEBUG: Verify the replacement actually happened
            const verifyPattern = new RegExp(
                `<Placemark>[\\s\\S]*?<description><!\\[CDATA\\[[\\s\\S]*?src="files/${escapedName}"[\\s\\S]*?\\]\\]></description>[\\s\\S]*?</Placemark>`,
                'i'
            );
            const verifyMatch = modifiedKml.match(verifyPattern);
            if (verifyMatch) {
                const verifiedName = verifyMatch[0].match(/<name><!\[CDATA\[([^\]]*)]]\><\/name>/i);
                if (verifiedName) {
                    console.log(`  DEBUG - Verified in KML: name is now "${verifiedName[1]}"`);
                }
            }
        });
