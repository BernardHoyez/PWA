...
async function downloadKml(modified = false) {
    ...
    try {
        ...
        // If modified version requested - DO THIS FIRST before replacing paths
        if (modified) {
            ...
            if (editWaypointsCheckbox.checked) {
                ...
                processedImages.forEach((img, index) => {
                    ...
                    // OruxMaps structure: chercher le Placemark avec l'image (chemin local)
                    const imgSrcPattern = `<img[^>]*src="${localPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>`;
                    
                    // Trouver le Placemark contenant cette image
                    const placemarkPattern = new RegExp(
                        `(<Placemark>[\\s\\S]*?<n><!\\[CDATA\\[)([^\\]]*)(\\]\\]></n>[\\s\\S]*?${imgSrcPattern}[\\s\\S]*?</Placemark>)`,
                        'i'
                    );
                    ...
                    if (match) {
                        const newPlacemark = match[1] + customName + match[3];
                        modifiedKml = modifiedKml.replace(match[0], newPlacemark);
                        console.log(`   ✓ Remplacé`);
                    } else {
                        console.log(`   ✗ Placemark non trouvé (cherchait: ${localPath})`);
                    }
                });
            }
            ...
            if (addImageLinksCheckbox.checked) {
                ...
                processedImages.forEach((img, index) => {
                    ...
                    // OruxMaps structure: chercher d'abord le chemin local dans img tag
                    const imgSrcPattern = new RegExp(
                        `<img[^>]*src="${localPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>`,
                        'i'
                    );
                    
                    if (!imgSrcPattern.test(modifiedKml)) {
                        console.log(`   ✗ Image src non trouvée (cherchait: ${localPath})`);
                        return;
                    }
                    
                    // Trouver la description contenant cette image
                    const descriptionPattern = new RegExp(
                        `(<description><!\\[CDATA\\[<table[^>]*>[\\s\\S]*?<img[^>]*src="${localPath.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>[\\s\\S]*?</table>)(\\s*)(\\]\\]></description>)`,
                        'i'
                    );
                    
                    const match = modifiedKml.match(descriptionPattern);
                    
                    if (match) {
                        // Vérifier si le lien n'existe pas déjà
                        if (match[0].includes('Agrandir') || match[0].includes('agrandir')) {
                            console.log(`   Déjà présent`);
                            return;
                        }
                        
                        // Ajouter le lien avant ]]></description>
                        ...
                        const newDescription = match[1] + linkHTML + match[2] + match[3];
                        
                        modifiedKml = modifiedKml.replace(match[0], newDescription);
        ...
        // NOW replace all file paths with GitHub URLs (AFTER modifications)
        processedImages.forEach(img => {
            const localPath = `files/${img.name}`;
            const githubUrl = githubBase + img.name;
