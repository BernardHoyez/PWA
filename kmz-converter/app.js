        ...
        if (modified) {
            ...
            if (editWaypointsCheckbox.checked) {
                ...
                processedImages.forEach((img, index) => {
                    ...
                    // OruxMaps structure: chercher le Placemark avec l'image
                    const imgSrcPattern = `<img[^>]*src="${githubUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>`;
            ...
            if (addImageLinksCheckbox.checked) {
                ...
                processedImages.forEach((img, index) => {
                    ...
                    // OruxMaps structure: chercher le src dans img tag
                    const imgSrcPattern = new RegExp(
                        `<img[^>]*src="files/${img.name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>`,
                        'i'
                    );
                    ...
                    if (imgSrcPattern.test(modifiedKml)) {
                        ...
                        const descriptionPattern = new RegExp(
                            `(<description><!\\[CDATA\\[<table[^>]*>[\\s\\S]*?<img[^>]*src="${githubUrl.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}"[^>]*>[\\s\\S]*?</table>)(\\s*)(\\]\\]></description>)`,
                            'i'
                        );
                        ...
                        console.log(`   ✗ Image src non trouvée`);
                    }
                });
            }
