...
class IgnibpApp {
    ...
    initMap() {
        ...
        // Couche IGN - Nouvelle URL Géoplateforme
        this.layers = {
            ign: L.tileLayer('https://data.geopf.fr/wmts?' +
    ...
    calculateIBP(stats) {
        // Algorithme IBP amélioré pour refléter la réalité de l'effort
        // Formule de base : IBP = (D + (D+ × K1) + (D- × K2)) × C
        // avec des coefficients ajustés pour être plus fidèle au ressenti
        
        const D = stats.distance; // distance en km
        const Dplus = stats.positiveElevation / 100; // dénivelé positif en hectomètres
        const Dminus = stats.negativeElevation / 100; // dénivelé négatif en hectomètres
        
        // Coefficient du D+ : augmenté car le dénivelé positif est le facteur le plus pénible
        const K1 = 3.5; // au lieu de 2.0 - impact plus fort du D+
        
        // Coefficient du D- : augmenté car la descente fatigue aussi (genoux, freinage)
        const K2 = 1.0; // au lieu de 0.5 - la descente compte davantage
        
        // Calcul de la pente moyenne
        const avgSlope = stats.positiveElevation / (stats.distance * 1000) * 100;
        
        // Coefficient multiplicateur basé sur la pente et la technicité
        // Plus la pente est forte, plus l'effort est démultiplié
        let C = 1.0;
        
        if (avgSlope > 20) {
            C = 2.2; // Pente extrême > 20% : effort très démultiplié
        } else if (avgSlope > 15) {
            C = 1.9; // Pente très forte 15-20%
        } else if (avgSlope > 12) {
            C = 1.7; // Pente forte 12-15%
        } else if (avgSlope > 10) {
            C = 1.5; // Pente soutenue 10-12%
        } else if (avgSlope > 8) {
            C = 1.35; // Pente moyenne 8-10%
        } else if (avgSlope > 6) {
            C = 1.2; // Pente modérée 6-8%
        } else if (avgSlope > 4) {
            C = 1.1; // Pente douce 4-6%
        }
        
        // Bonus de longueur : les longues randonnées épuisent davantage
        // Au-delà de 15 km, chaque km supplémentaire coûte plus cher
        let lengthBonus = 1.0;
        if (D > 25) {
            lengthBonus = 1.25; // Très longue distance
        } else if (D > 20) {
            lengthBonus = 1.20;
        } else if (D > 15) {
            lengthBonus = 1.15;
        ...
        // Facteur d'irrégularité : si beaucoup de variations de pente
        // Calculer l'écart-type des pentes
        const slopeMean = stats.slopes.reduce((a, b) => a + b, 0) / stats.slopes.length;
        const slopeVariance = stats.slopes.reduce((sum, slope) => {
            return sum + Math.pow(slope - slopeMean, 2);
        }, 0) / stats.slopes.length;
        const slopeStdDev = Math.sqrt(slopeVariance);
        
        // Si forte variation de pente (nombreuses montées/descentes), c'est plus fatiguant
        let irregularityFactor = 1.0;
        if (slopeStdDev > 8) {
            irregularityFactor = 1.15; // Profil très irrégulier
        } else if (slopeStdDev > 5) {
            irregularityFactor = 1.10; // Profil irrégulier
        } else if (slopeStdDev > 3) {
            irregularityFactor = 1.05; // Profil un peu irrégulier
        }
        
        // Calcul final avec tous les facteurs
        const baseIBP = (D + (Dplus * K1) + (Dminus * K2)) * C;
        const ibp = Math.round(baseIBP * lengthBonus * irregularityFactor);
    ...
    getDifficultyLevel(ibp) {
        // Seuils ajustés pour correspondre au ressenti
        if (ibp < 30) return 'Très facile';
        if (ibp < 60) return 'Facile';
        if (ibp < 90) return 'Modéré';
        if (ibp < 130) return 'Difficile';
        if (ibp < 180) return 'Très difficile';
        return 'Extrêmement difficile';
    }
    
    getDifficultyDescription(ibp) {
        if (ibp < 30) return 'Promenade accessible à tous, très peu d\'effort physique';
        if (ibp < 60) return 'Randonnée facile, effort léger, accessible aux débutants';
        if (ibp < 90) return 'Effort modéré, bonne condition physique de base requise';
        if (ibp < 130) return 'Randonnée difficile, effort soutenu, préparation recommandée';
        if (ibp < 180) return 'Très difficile, effort intense, expérience nécessaire';
        return 'Extrêmement difficile, effort maximal, réservé aux experts confirmés';
    }
