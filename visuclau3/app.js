    ...
    showPOIList(pois, coords) {
        let listHtml = `
            <div class="custom-popup">
                ...
                <div id="debug-info" style="font-size:0.8rem;color:#666;padding:0.5rem;background:#f0f0f0;margin-bottom:0.5rem;">
                    Status: En attente de clic...
                </div>
                <div class="poi-list">
        `;
        ...
        const popup = L.popup({
            ...
            closeButton: true,
            autoClose: false // Important: ne pas fermer automatiquement
        })
        ...
        const updateDebug = (message) => {
            ...
            if (debugEl) {
                debugEl.textContent = `Status: ${message}`;
                debugEl.style.background = '#e6f3ff';
                debugEl.style.color = '#0066cc';
            }
        };

        // Gestion simple et directe des clics
        popup.on('add', () => {
            updateDebug('Popup ajouté, attachement événements...');
            
            // Délai plus long pour être sûr
            setTimeout(() => {
                const poiItems = document.querySelectorAll('.poi-item[data-poi-id]');
                updateDebug(`${poiItems.length} items trouvés, ajout événements...`);
                
                poiItems.forEach((item) => {
                    const poiId = parseInt(item.getAttribute('data-poi-id'));
                    const selectedPoi = pois.find(p => p.id === poiId);
                    
                    if (!selectedPoi) {
                        updateDebug(`Erreur: POI ${poiId} non trouvé`);
                        return;
                    }
                    
                    // Gestionnaire de clic simple
                    const handleClick = () => {
                        updateDebug(`Clic détecté sur POI ${poiId} - ${selectedPoi.title}`);
                        
                        // Fermer ce popup
                        this.map.closePopup();
                        
                        // Ouvrir immédiatement le nouveau popup
                        setTimeout(() => {
                            this.showPOIPopup(selectedPoi);
                        }, 100);
                    };
                    
                    // Multiple event listeners pour compatibilité maximale
                    item.addEventListener('click', handleClick, { passive: false });
                    item.addEventListener('touchstart', handleClick, { passive: false });
                    item.addEventListener('mousedown', handleClick, { passive: false });
                    
                    // Feedback visuel au clic
                    item.addEventListener('touchstart', () => {
                        item.style.background = '#2563eb';
                        item.style.color = 'white';
                        updateDebug(`Touch sur POI ${poiId}`);
                    });
                ...
                updateDebug(`Événements attachés à ${poiItems.length} items - Prêt!`);
