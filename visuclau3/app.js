class FieldGuideApp {
    ...
    async loadMediaFiles(poi, folderPath, item) {
        const mediaTypes = [
            { key: 'image', prop: 'imageUrl', mimeTypes: ['image/jpeg', 'image/jpg', 'image/png', 'image/gif', 'image/webp'] },
            { key: 'video', prop: 'videoUrl', mimeTypes: ['video/mp4', 'video/webm', 'video/ogg'] },
            { key: 'audio', prop: 'audioUrl', mimeTypes: ['audio/mpeg', 'audio/mp3', 'audio/wav', 'audio/ogg'] }
        ];
            ...
            if (item[media.key]) {
                try {
                    ...
                    if (mediaFile) {
                        ...
                        // Déterminer le type MIME basé sur l'extension si le blob n'a pas de type
                        ...
                        if (!mimeType || mimeType === 'application/octet-stream') {
                            ...
                            switch (extension) {
                                ...
                                case 'wav':
                                    ...
                                    break;
                                case 'ogg':
                                    if (media.key === 'video') mimeType = 'video/ogg';
    ...
    showPOIPopup(poi) {
        ...
        // Médias avec gestion d'erreurs améliorée
        let mediaContent = '';
        ...
        if (poi.imageUrl) {
            ...
            mediaContent += `
                <div style="margin-bottom: 1rem;">
                    <h4 style="margin: 0 0 0.5rem 0; color: #555; font-size: 0.9rem;">📸 Image</h4>
                    <img src="${poi.imageUrl}" 
                         alt="Image ${poi.title}" 
                         style="width: 100%; max-width: 100%; height: auto; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); cursor: pointer;" 
                         onclick="this.style.transform = this.style.transform ? '' : 'scale(1.02)'; window.open(this.src, '_blank');"
                         onload="console.log('Image chargée avec succès');"
                         onerror="console.error('Erreur chargement image:', this.src); this.style.display='none'; this.nextElementSibling.style.display='block';">
                    <div style="display: none; padding: 1rem; background: #fee2e2; color: #dc2626; border-radius: 8px; text-align: center;">
                        ...
                        <small style="font-size: 0.8rem;">Vérifiez le format et la taille du fichier</small>
                    </div>
                </div>
            `;
        }
