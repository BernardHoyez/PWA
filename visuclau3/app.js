class FieldGuideApp {
    ...
    showPOIList(pois, coords) {
        ...
        const popup = L.popup({
            maxWidth: Math.min(window.innerWidth * 0.85, 400),
        ...
        // Ajouter les événements après que le popup soit dans le DOM
        popup.on('add', () => {
            ...
            setTimeout(() => {
                ...
                poiItems.forEach(item => {
                    item.addEventListener('click', (e) => {
                        ...
                        const selectedPoi = this.pois.find(p => p.id === poiId);
                        if (selectedPoi) {
                            this.map.closePopup(); // Fermer la liste
                            this.showPOIPopup(selectedPoi); // Ouvrir le POI
                        }
                    });
                });
            }, 100);
