...
async function renderPOIs() {
  ...
  visit.pois.forEach(p => {
    const key = isoCoordKey(p.lat, p.lon);
  ...
  visit.pois.forEach(poi => {
    ...
    if (poi.image) {
      ...
      else html += `<div>(vidéo manquante : ${escapeHtml(poi.video.name)})</div>`;
    }
    html += `</div>`;
  ...
  // Changer la couleur du marqueur (rouge ou bleu)
  Object.values(markers).forEach(m => {
    const col = m.color === 'red' ? '#d33' : '#33a1ff';
    const svg = encodeURIComponent(
      `<svg xmlns='http://www.w3.org/2000/svg' width='36' height='36'>
         <circle cx='18' cy='12' r='8' fill='${col}' stroke='#000' stroke-width='1'/>
         <rect x='16' y='18' width='4' height='12' fill='${col}'/>
       </svg>`
    );
    const icon = L.icon({
...
async function handleZipFile(file) {
  try {
    ...
    zipEntries = z.files;
    console.log('Fichiers trouvés dans le zip :', Object.keys(z.files));

    // Recherche du visit.json
    const visitFiles = Object.keys(z.files).filter(n => /visit\.json$/i.test(n));
    if (visitFiles.length === 0) {
      alert('visit.json manquant dans le ZIP.');
      return;
    }
    ...
    for (const poi of visit.pois || []) {
      for (const k of ['image', 'audio', 'video']) {
        if (poi[k] && poi[k].name) {
          ...
          const found = possiblePaths.find(p => z.files[p]);
          if (found) {
            const blob = await z.files[found].async('blob');
            mediaBlobs[poi[k].name] = URL.createObjectURL(blob);
          } else {
            console.warn('Média manquant dans le ZIP :', poi[k].name);
          }
        }
      }
    }
    ...
    alert('Erreur lors du traitement du ZIP. Voir la console pour plus de détails.');
  }
}

// --- Sauvegarde du ZIP modifié ---
async function saveModifiedZip() {
  ...
  try {
    ...
    alert('Erreur lors de la création du ZIP modifié.');
  }
}
...
window.addEventListener('load', () => {
  ...
  if ('serviceWorker' in navigator) {
    navigator.serviceWorker.register('service-worker.js')
      .catch(() => console.warn('Service worker non enregistré.'));
  }
