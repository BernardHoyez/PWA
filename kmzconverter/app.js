...
downloadKmlBtn.addEventListener('click', downloadKml);
...
async function downloadKml() {
    ...
    try {
        ...
        processedImages.forEach(img => {
            // Replace files/imagename.jpg with GitHub URL
            const localPath = `files/${img.name}`;
            const githubUrl = githubBase + img.name;
            
            // Handle different possible formats in KML
        ...
        link.download = `${folderName}-github.kml`;
        link.click();
        ...
        downloadKmlBtn.innerHTML = `
            ...
            Télécharger le KML avec liens GitHub
        `;
    } catch (error) {
