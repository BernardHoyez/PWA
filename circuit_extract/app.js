...
function initMap() {
  ...
  // Injecter l'overlay directement dans le container Leaflet (map.getContainer())
  // APRES que Leaflet a injecté tous ses propres divs.
  // map.getContainer() === #leaflet-map, l'overlay sera le DERNIER enfant
  // donc au-dessus de tout dans ce stacking context.
...
function pipMove(e) {
  const c   = document.getElementById('pip-canvas');
  const rec = c.getBoundingClientRect();
  // Convertir coordonnées CSS → coordonnées pixels canvas intrinsèques
  const scaleX = c.width  / rec.width;
  const scaleY = c.height / rec.height;
  const mx = Math.round((e.clientX - rec.left) * scaleX);
  const my = Math.round((e.clientY - rec.top)  * scaleY);
  updatePip(c, mx, my);
}
