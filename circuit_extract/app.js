...
function chainGreedy(sk,W,H){
  ...
  // Voisins 8-connexes dans le squelette
  function nb(idx){
  ...
  // ── Étape 1 : collecter tous les segments connexes du squelette ──────────
  // Un segment = suite de pixels 8-connexes partant d'une extrémité (degré 1)
  // ou d'un pixel quelconque si le tracé est une boucle fermée.
  const visited=new Uint8Array(sk.length);
  const segments=[];                 // tableau de tableaux d'indices

  // Trouver les extrémités (degré 1) comme points de départ préférés
  const endpoints=[];
  for(let i=0;i<sk.length;i++){if(sk[i]&&nb(i).length===1)endpoints.push(i);}

  function traceFrom(start){
    const seg=[];let cur=start;
    while(cur>=0){
      if(visited[cur])break;
      visited[cur]=1;seg.push(cur);
      const cands=nb(cur).filter(n=>!visited[n]);
      if(!cands.length)break;
      // Préférer la direction qui continue le tracé
      if(seg.length>=2){
        const pi=seg[seg.length-2],ci=seg[seg.length-1];
        const ddx=(ci%W)-(pi%W),ddy=((ci/W)|0)-((pi/W)|0);
        cands.sort((a,b)=>{
          const ax=(a%W)-(ci%W),ay=((a/W)|0)-((ci/W)|0);
          const bx=(b%W)-(ci%W),by=((b/W)|0)-((ci/W)|0);
  ...
  // Tracer depuis chaque extrémité non visitée
  for(const ep of endpoints){
    if(!visited[ep]){const seg=traceFrom(ep);if(seg.length>=2)segments.push(seg);}
  }
  // Puis ramasser les pixels restants (boucles fermées sans extrémité)
  for(let i=0;i<sk.length;i++){
    if(sk[i]&&!visited[i]){const seg=traceFrom(i);if(seg.length>=2)segments.push(seg);}
  }

  if(!segments.length)return[];
  logMsg(`Segments squelette : ${segments.length}`);

  // ── Étape 2 : relier les segments dans le bon ordre (TSP greedy) ─────────
  // Chaque segment a deux extrémités. On relie bout à bout en minimisant la
  // distance entre la fin du segment courant et le début/fin du suivant.
  // Tolérance de jonction : on accepte un gap jusqu'à MAX_GAP pixels.
  const MAX_GAP=200; // pixels — tolérance de jonction entre fragments

  function dist2pts(ax,ay,bx,by){return(ax-bx)**2+(ay-by)**2;}
  function endPts(seg){
    const a=seg[0],b=seg[seg.length-1];
    return{ax:a%W,ay:(a/W)|0,bx:b%W,by:(b/W)|0};
  }

  const used=new Uint8Array(segments.length);

  // Trouver le segment le plus long comme point de départ
  let startIdx=0;
  for(let j=1;j<segments.length;j++) if(segments[j].length>segments[startIdx].length) startIdx=j;
  let chain=segments[startIdx].map(i=>[i%W,(i/W)|0]);
  used[startIdx]=1;
  let joined=1;

  // Liaison greedy bidirectionnelle :
  // À chaque itération, on cherche le segment non utilisé dont UNE extrémité
  // est la plus proche de la TÊTE ou de la QUEUE de la chaîne courante.
  while(joined<segments.length){
    const head=chain[0], tail=chain[chain.length-1];
    let bestDist=Infinity,bestIdx=-1,bestAttachToTail=true,bestFlip=false;

    for(let j=0;j<segments.length;j++){
      if(used[j])continue;
      const{ax,ay,bx,by}=endPts(segments[j]);

      // Depuis la queue
      const dTailStart=dist2pts(tail[0],tail[1],ax,ay);
      const dTailEnd  =dist2pts(tail[0],tail[1],bx,by);
      if(dTailStart<bestDist){bestDist=dTailStart;bestIdx=j;bestAttachToTail=true; bestFlip=false;}
      if(dTailEnd  <bestDist){bestDist=dTailEnd;  bestIdx=j;bestAttachToTail=true; bestFlip=true;}

      // Depuis la tête (on retourne la chaîne si nécessaire)
      const dHeadStart=dist2pts(head[0],head[1],ax,ay);
      const dHeadEnd  =dist2pts(head[0],head[1],bx,by);
      if(dHeadStart<bestDist){bestDist=dHeadStart;bestIdx=j;bestAttachToTail=false;bestFlip=true;}
      if(dHeadEnd  <bestDist){bestDist=dHeadEnd;  bestIdx=j;bestAttachToTail=false;bestFlip=false;}
    }

    if(bestIdx<0||Math.sqrt(bestDist)>MAX_GAP)break;

    used[bestIdx]=1;
    const seg=segments[bestIdx];
    const pts=seg.map(i=>[i%W,(i/W)|0]);
    const ptsOrdered=bestFlip?pts.slice().reverse():pts;

    if(bestAttachToTail){
      chain=chain.concat(ptsOrdered);
    } else {
      // Attacher en tête : inverser la chaîne, ajouter, réinverser
      chain=ptsOrdered.concat(chain);
    }
    joined++;
  }

  logMsg(`Segments reliés : ${joined}/${segments.length}${joined<segments.length?' ('+( segments.length-joined)+' ignorés — gap >'+MAX_GAP+'px)':''}`);
  return chain;
}
