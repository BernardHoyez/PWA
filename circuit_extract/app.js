...
async function runExtraction(){
  ...
  // Déduplication APRÈS RDP — à ce stade les points sont espacés de plusieurs px.
  // On supprime les doublons GPS : points consécutifs trop proches (< 2px après RDP)
  // ou retours en arrière évidents (aller-retour dans un rayon < 3*eps).
  const minDist = Math.max(3, eps * 2);
  pts = dedupAfterRdp(pts, minDist);
...
// Déduplication post-RDP : supprime les points qui forment un aller-retour
// visible. Après RDP, les points sont espacés — on cherche si un point P[i]
// se retrouve à moins de minDist px d'un point P[j] avec j < i-2.
// Si oui, on coupe la boucle en sautant de j à i directement.
function dedupAfterRdp(pts, minDist) {
  ...
  while (i < pts.length) {
    const [cx, cy] = pts[i];
    // Chercher dans les 20 derniers points si ce point est un retour
    let jumpTo = -1;
    const wStart = Math.max(0, out.length - 20);
    for (let j = wStart; j < out.length - 1; j++) {
      const dx = cx - out[j][0], dy = cy - out[j][1];
      if (dx*dx + dy*dy < md2) { jumpTo = j; break; }
    }
    if (jumpTo >= 0) {
      // On a détecté un retour vers out[jumpTo] — couper la boucle :
      // tronquer out jusqu'à jumpTo et reprendre depuis i
      out.splice(jumpTo + 1);
    } else {
      out.push(pts[i]);
    }
