...
function redrawWand(){
  ...
  for(let i=0;i<imgW*imgH;i++)if(selMask[i]){id.data[i*4]=0;id.data[i*4+1]=200;id.data[i*4+2]=80;id.data[i*4+3]=160;}
...
function chainGreedy(sk,W,H){
  ...
  function nb(idx){const x=idx%W,y=(idx/W)|0,r=[];for(const[dy,dx]of d8){const nx=x+dx,ny=y+dy;if(nx>=0&&nx<W&&ny>=0&&ny<H&&sk[ny*W+nx])r.push(ny*W+nx);}return r;}
  let start=-1;
  for(let i=0;i<sk.length;i++){if(!sk[i])continue;if(nb(i).length===1){start=i;break;}}
  if(start<0)for(let i=0;i<sk.length;i++)if(sk[i]){start=i;break;}
  if(start<0)return[];
  const vis=new Uint8Array(sk.length),chain=[];let cur=start;
  while(cur>=0){
    vis[cur]=1;chain.push([cur%W,(cur/W)|0]);
    const cands=nb(cur).filter(n=>!vis[n]);if(!cands.length)break;
    if(chain.length>=2){
      const prev=chain[chain.length-2],curr=chain[chain.length-1];
      const ddx=curr[0]-prev[0],ddy=curr[1]-prev[1];
      cands.sort((a,b)=>{const ax=(a%W)-curr[0],ay=((a/W)|0)-curr[1],bx=(b%W)-curr[0],by=((b/W)|0)-curr[1];return(bx*ddx+by*ddy)-(ax*ddx+ay*ddy);});
    }
    cur=cands[0];
  }return chain;
}
...
function renderResult(chain,mask,W,H){
  ...
  for(let i=0;i<W*H;i++)if(mask[i]){id.data[i*4]=0;id.data[i*4+1]=200;id.data[i*4+2]=80;id.data[i*4+3]=100;}
