...
async function runExtraction(){
  ...
  setP(30,'Squelettisation…');await dl(40);
  ...
  // Dédupliquer : supprimer les allers-retours résiduels dus aux rails parallèles.
  // Méthode : fenêtre glissante — si un point est très proche d'un point récent
  // (dans les N derniers), c'est un retour en arrière → on l'ignore.
  const deduped = dedupChain(chain, 8);
  logMsg(`Après déduplication : ${deduped.length} pts`);

  setP(78,'Simplification…');await dl(10);
  const eps=parseInt(document.getElementById('simplify').value);
  let pts=deduped;
  if(eps>0&&pts.length>5){pts=rdp(pts,eps);logMsg(`RDP ε=${eps}: ${deduped.length}→${pts.length}`);}
...
// Déduplique la chaîne en supprimant les allers-retours.
// Principe : pour chaque point P[i], on vérifie s'il est très proche
// d'un point P[j] avec j < i-1 (retour en arrière).
// Si oui, on cherche la meilleure façon de "couper" la boucle parasite.
// Implémentation simple et robuste : fenêtre glissante de W points.
// Si P[i] est à moins de THRESH px d'un des W points précédents → doublon → sauter.
function dedupChain(chain, win) {
  if (chain.length < 4) return chain;
  const THRESH = 6; // distance en pixels pour considérer un retour
  const out = [chain[0]];
  ...
  for (let i = 1; i < chain.length; i++) {
    ...
    // Chercher dans la fenêtre des points précédents (exclu le dernier)
    const wStart = Math.max(0, out.length - win - 1);
    for (let j = wStart; j < out.length - 1; j++) {
      const dx = cx - out[j][0], dy = cy - out[j][1];
      if (dx*dx + dy*dy < THRESH*THRESH) {
        isDup = true;
        break;
      }
    }
    if (!isDup) out.push(chain[i]);
  }
  return out;
}
