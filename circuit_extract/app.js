...
async function runExtraction(){
  ...
  // Érosion itérative pour ramener le trait à ~1px avant squelettisation.
  // Élimine les "rails parallèles" qui causent des doublons dans le KML.
  setP(22,'Amincissement du masque…');await dl(20);
  mask=erodeToThin(mask,W,H);
  let eCnt=0;for(let i=0;i<mask.length;i++)if(mask[i])eCnt++;
  logMsg(`Après amincissement : ${eCnt.toLocaleString()} px`);

  setP(38,'Squelettisation…');await dl(40);
  ...
  setP(62,'Chaînage…');await dl(20);
  const chain=chainGreedy(sk,W,H);
  logMsg(`Chaîne brute : ${chain.length} pts`);
  if(chain.length<2){setP(100,'⚠ Chaîne vide. Essayez "Fermeture des gaps".');return;}

  // Filtrer les oscillations : supprimer les points qui reviennent en arrière
  // sur moins de OWIN pixels (signe d'un aller-retour sur des rails parallèles)
  const OWIN=4; // fenêtre de détection en pixels
  const filtered=filterOscillations(chain,OWIN);
  logMsg(`Après filtrage oscillations : ${filtered.length} pts`);

  setP(78,'Simplification…');await dl(10);
  const eps=parseInt(document.getElementById('simplify').value);
  let pts=filtered;
  if(eps>0&&pts.length>5){pts=rdp(pts,eps);logMsg(`RDP ε=${eps}: ${filtered.length}→${pts.length}`);}
...
// Érosion morphologique d'1px : supprime les pixels dont au moins un voisin
// 4-connexe est absent. Répétée plusieurs fois, réduit l'épaisseur du trait.
function erode1px(mask,W,H){
  const out=new Uint8Array(W*H);
  for(let y=1;y<H-1;y++) for(let x=1;x<W-1;x++){
    if(!mask[y*W+x])continue;
    // Garder seulement si les 4 voisins directs sont aussi dans le masque
    if(mask[(y-1)*W+x]&&mask[(y+1)*W+x]&&mask[y*W+x-1]&&mask[y*W+x+1]) out[y*W+x]=1;
  }
  return out;
}

// Érosion répétée jusqu'à ce que l'épaisseur moyenne soit proche de 1px.
// Stoppe automatiquement quand enlever 1 px de plus réduirait trop le masque.
function erodeToThin(mask,W,H){
  let cur=mask;
  for(let iter=0;iter<8;iter++){
    const next=erode1px(cur,W,H);
    let cntCur=0,cntNext=0;
    for(let i=0;i<cur.length;i++){if(cur[i])cntCur++;if(next[i])cntNext++;}
    if(cntNext<10) break; // trop érodé
    // S'arrêter si l'érosion supprime moins de 15% des pixels restants
    // (on a atteint un trait fin)
    if((cntCur-cntNext)/cntCur<0.15) break;
    cur=next;
  }
  return cur;
}

// Filtre les oscillations dans la chaîne de points :
// un "doublon" se manifeste comme un zigzag où le tracé revient en arrière
// sur une très courte distance. On détecte ces retours en comparant
// la direction instantanée sur une fenêtre glissante.
function filterOscillations(chain,winSize){
  if(chain.length<winSize*2) return chain;
  const out=[chain[0]];
  for(let i=1;i<chain.length;i++){
    const p=out[out.length-1],c=chain[i];
    // Calculer le vecteur depuis le point précédent
    const dx=c[0]-p[0],dy=c[1]-p[1];
    // Regarder la direction générale sur les 'winSize' derniers points
    if(out.length>=winSize){
      const ref=out[out.length-winSize];
      const gdx=p[0]-ref[0],gdy=p[1]-ref[1];
      const dot=dx*gdx+dy*gdy;
      const lenG=Math.sqrt(gdx*gdx+gdy*gdy);
      // Si le nouveau point va dans le sens opposé à la direction générale
      // ET que le mouvement est très petit → oscillation → ignorer
      if(lenG>0&&dot<0&&Math.sqrt(dx*dx+dy*dy)<winSize*1.5) continue;
    }
    out.push(c);
  }
  return out;
