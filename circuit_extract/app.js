...
function initMap() {
  ...
  // S'assurer que l'overlay est TOUJOURS le dernier enfant de map-pane
  // (Leaflet injecte ses divs dans #leaflet-map au moment de L.map(),
  //  l'overlay doit rester après dans l'ordre DOM)
  const mapPane = document.getElementById('map-pane');
  const overlay = document.getElementById('map-pick-overlay');
  const hint    = document.getElementById('map-hint');
  mapPane.appendChild(overlay);   // déplace à la fin
  mapPane.appendChild(hint);      // garde hint au-dessus de l'overlay
}
...
function enableMapPicking() {
  const overlay = document.getElementById('map-pick-overlay');
  // Retirer tout écouteur précédent avant d'en ajouter un nouveau
...
function onOverlayClick(e) {
  // S'assurer que l'overlay est bien retiré même si pickState est invalide
  disableMapPicking();
  if (!pickState || pickState.step !== 'map') return;

  // Conversion coordonnées pixel écran → lat/lon
  // On utilise getBoundingClientRect du conteneur Leaflet (pas map-pane)
