...
function chainGreedy(sk,W,H){
  ...
  // ── Étape 1 : extraire TOUS les segments connexes ─────────────────────────
  // Règle stricte : chaque pixel n'appartient qu'à UN SEUL segment.
  // On part toujours d'une extrémité (degré 1) si possible.
  ...
  // Parcourir d'abord les extrémités (degré 1), puis les pixels restants
  for(let i=0;i<sk.length;i++){
    if(sk[i]&&!visited[i]&&nb(i).length===1){
      const seg=traceFrom(i);if(seg.length>=2)segments.push(seg);
    }
  }
  for(let i=0;i<sk.length;i++){
    if(sk[i]&&!visited[i]){
  ...
  // ── Étape 2 : relier les segments en QUEUE UNIQUEMENT (pas de doublons) ───
  //
  // Principe : on maintient une chaîne dont on étend UNIQUEMENT la queue.
  // Pour chaque nouveau segment, on choisit l'orientation telle que
  // l'extrémité la plus proche de la queue actuelle devient le PREMIER point
  // du segment ajouté. La queue avance toujours dans un sens → pas de retour.
  //
  // Choix du segment de départ : le plus long parmi ceux qui commencent
  // par une vraie extrémité du squelette (degré 1), orienté depuis cette extrémité.

  const MAX_GAP=250;
  function d2(ax,ay,bx,by){return(ax-bx)**2+(ay-by)**2;}
  function ptOf(idx){return[idx%W,(idx/W)|0];}

  // Trouver le segment de départ optimal
  const used=new Uint8Array(segments.length);
  let startIdx=0;
  for(let j=1;j<segments.length;j++){
    if(segments[j].length>segments[startIdx].length) startIdx=j;
  }
  // Orienter : on veut commencer par l'extrémité de degré 1 si possible
  {
    const seg=segments[startIdx];
    const degA=nb(seg[0]).length, degB=nb(seg[seg.length-1]).length;
    // Si B est une extrémité (deg 1) et A ne l'est pas → inverser
    if(degB===1&&degA!==1) segments[startIdx]=seg.slice().reverse();
  }

  let chain=segments[startIdx].map(ptOf);
  used[startIdx]=1;
  let joined=1;

  while(joined<segments.length){
    // Extrémité courante de la chaîne (queue uniquement)
    ...
    for(let j=0;j<segments.length;j++){
      ...
      const [ax,ay]=ptOf(seg[0]);
      const [bx,by]=ptOf(seg[seg.length-1]);
      // Distance entre la queue et chaque extrémité du segment candidat
      const dA=d2(tail[0],tail[1],ax,ay);
      const dB=d2(tail[0],tail[1],bx,by);
      if(dA<bestDist){bestDist=dA;bestIdx=j;bestFlip=false;} // commencer par A
      if(dB<bestDist){bestDist=dB;bestIdx=j;bestFlip=true;}  // commencer par B (flip)
    }

    if(bestIdx<0||Math.sqrt(bestDist)>MAX_GAP)break;

    used[bestIdx]=1;
    const pts=segments[bestIdx].map(ptOf);
    // bestFlip=true → l'extrémité B est la plus proche → on inverse pour commencer par B
    chain=chain.concat(bestFlip?pts.slice().reverse():pts);
    joined++;
  }

  const ignored=segments.length-joined;
  logMsg(`Segments reliés : ${joined}/${segments.length}${ignored>0?' — '+ignored+' ignoré(s), gap >'+MAX_GAP+'px':''}`);
  return chain;
}
