...
function enableMapPicking() {
  // Désactiver drag + doubleClickZoom pour que le clic soit capté proprement
  map.dragging.disable();
  map.doubleClickZoom.disable();
  // Forcer le curseur crosshair sur toute la carte
  document.getElementById('leaflet-map').style.cursor = 'crosshair';
  map.getContainer().querySelectorAll('*').forEach(el => el.style.cursor = 'crosshair');
  // Écouter le prochain clic une seule fois
...
function disableMapPicking() {
  ...
  map.getContainer().querySelectorAll('*').forEach(el => el.style.cursor = '');
}
...
function onMapClick(e) {
  ...
  const idx = pickState.idx;
  gcps[idx].lat = e.latlng.lat;
  gcps[idx].lon = e.latlng.lng;
  pickState = null;
...
function delGCP(i) {
  ...
  // Si on supprime pendant une phase de pointage carte, restaurer l'état de la carte
