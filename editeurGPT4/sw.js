    ...
    if(!hasCoords){
      ...
      GeoPicker.show({
        ...
        onConfirm: function(lat,lng){
            ...
            if(typeof i !== 'undefined' && points && points[i]) {
                points[i].lat = lat;
                points[i].lng = lng;
                var latSpan = document.getElementById('lat-'+i);
                var lngSpan = document.getElementById('lng-'+i);
                if(latSpan) latSpan.innerText = points[i].lat.toFixed(6);
                if(lngSpan) lngSpan.innerText = points[i].lng.toFixed(6);
            }
