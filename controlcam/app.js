...
function RemoteCameraApp() {
  ...
  const initCamera = () => {
    ...
    console.log('Initialisation caméra, ID:', id);
    setDeviceId(id);
    setMode('camera');
  };
  ...
  const startCamera = async () => {
    ...
    try {
      ...
      console.log('Caméra obtenue!');
      setStream(mediaStream);
      
      if (videoRef.current) {
        videoRef.current.srcObject = mediaStream;
        console.log('Flux attaché à la vidéo');
      } else {
  ...
  if (mode === 'camera') {
    console.log('Affichage mode caméra, deviceId:', deviceId, 'error:', error);
