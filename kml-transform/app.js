...
githubRepoSelect.addEventListener('change', () => {
    if (githubRepoSelect.value === 'custom') {
        customRepoGroup.style.display = 'block';
        githubBaseInput.value = '';
        githubBaseInput.focus();
    } else {
        customRepoGroup.style.display = 'none';
        githubBaseInput.value = githubRepoSelect.value;
    }
    updateFolderPreview();
});
...
// DEBUG button
const debugKmlBtn = document.getElementById('debug-kml-btn');
debugKmlBtn.addEventListener('click', () => {
    const kml = localStorage.getItem('debug_modified_kml');
    if (!kml) {
        alert('Générez d\'abord un KMZ pour voir le KML modifié');
        return;
    }
    ...
    link.download = 'debug_modified.kml';
    link.click();
        ...
        item.innerHTML = `
            ...
            <div class="waypoint-fields">
                ...
                <div class="field-group">
                    <label>Nom du waypoint * (max 50 caractères)</label>
                    <input 
                        type="text" 
                        class="waypoint-name" 
                        value="${data.name}" 
                ...
                <div class="field-group">
                    <label>Commentaire (optionnel, max 500 caractères)</label>
                    <textarea 
...
async function downloadModifiedKmz() {
    ...
    try {
        ...
        // CRITICAL FIX: Use replace() with callback to modify each Placemark ONCE
        processedImages.forEach(img => {
            ...
            // Pattern: Find the FIRST Placemark containing THIS specific image
            const placemarkPattern = new RegExp(
                `<Placemark>[\\s\\S]*?<description><!\\[CDATA\\[[\\s\\S]*?src="files/${escapedName}"[\\s\\S]*?\\]\\]></description>[\\s\\S]*?</Placemark>`,
                'i'
            );
            
            // Replace using callback (only replaces FIRST match)
            let found = false;
            modifiedKml = modifiedKml.replace(placemarkPattern, (match) => {
                found = true;
                console.log(`  ✓ Found Placemark (length: ${match.length})`);
                
                // Replace name
                const namePattern = /(<n><!\[CDATA\[)([^\]]*)(]\]><\/name>)/i;
                let updated = match.replace(namePattern, `$1${data.name}$3`);
                console.log(`  ✓ Name replaced`);
                
                // Add comment
                if (data.comment) {
                    const descPattern = /(<description><!\[CDATA\[)/i;
                    updated = updated.replace(descPattern, 
        ...
        // DEBUG: Save to localStorage
        try {
            localStorage.setItem('debug_modified_kml', modifiedKml);
            console.log('✓ KML sauvegardé dans localStorage pour inspection');
        } catch(e) {}
    ...
    try {
        ...
        processedImages.forEach(img => {
            ...
            if (placemarkMatch) {
                ...
                // Replace the name
                const nameRegex = /(<name><!\[CDATA\[)([^\]]*)(]]\><\/name>)/i;
