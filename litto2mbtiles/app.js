  ...
  try {
    ...
    for (let i=0; i<entries.length; i+=BATCH) {
      ...
      for (const [key,data] of entries.slice(i,i+BATCH)) {
        ...
        st.run([z, x, (1<<z)-1-y, data]); // inversion Y pour MBTiles
      }
      db.run('COMMIT');
    ...
    const name = `litto2mbtiles_${(S.ptStart?.name||'A').replace(/\s+/g,'_')}`
               + `_${(S.ptEnd?.name||'B').replace(/\s+/g,'_')}`
               + `_z${S.zoomMin}-${S.zoomMax}.mbtiles`;
    const a = document.createElement('a');
    a.href     = URL.createObjectURL(new Blob([raw],{type:'application/octet-stream'}));
    a.download = name;
    a.click();
    URL.revokeObjectURL(a.href);
    log(`Exporté : ${(raw.byteLength/1048576).toFixed(1)} Mo — ${entries.length} tuiles`, 'success');
  } catch(e) {
