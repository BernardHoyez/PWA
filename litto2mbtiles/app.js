  ...
  for (let z = zMin; z <= zMax; z++) {
    ...
    for (let x = nw.x; x <= se.x; x++)
      for (let y = nw.y; y <= se.y; y++)
        if (bboxOverlap(pb, tileBBox(x,y,z))) result.push([z,x,y]);
  }
  return result;
}
...
async function exportMBTiles() {
  ...
  try {
    if (!S.SQL) {
      ...
      if (!fn) throw new Error('sql.js absent');
    ...
    [
      ['name',       `litto2mbtiles — ${S.ptStart?.name||'?'} → ${S.ptEnd?.name||'?'}`],
      ['description',`Ortho ${S.orthoSrc.toUpperCase()} — buffer ${S.bufferKm*1000}m — Z${S.zoomMin}-${S.zoomMax}`],
      ['format',      S.orthoSrc==='ign'?'jpeg':'png'],
      ['bounds',      bb.join(',')],
      ['center',      `${ct[0].toFixed(6)},${ct[1].toFixed(6)},${S.zoomMax-1}`],
      ['minzoom',     String(S.zoomMin)],
      ['maxzoom',     String(S.zoomMax)],
    ].forEach(([k,v]) => db.run('INSERT INTO metadata VALUES(?,?)',[k,v]));

    const st = db.prepare('INSERT OR REPLACE INTO tiles VALUES(?,?,?,?)');
    ...
    for (let i=0; i<entries.length; i+=BATCH) {
      ...
      for (const [key,data] of entries.slice(i,i+BATCH)) {
        ...
        st.run([z, x, (1<<z)-1-y, data.buffer]); // inversion Y pour MBTiles — ArrayBuffer requis
      }
      db.run('COMMIT');
      const pct = Math.min(100,Math.round((i+BATCH)/entries.length*100));
      $('prog-bar').style.width = `${pct}%`;
      $('prog-pct').textContent = `${pct}%`;
      $('prog-detail').textContent = `SQL ${Math.min(i+BATCH,entries.length)}/${entries.length}`;
      await new Promise(r=>setTimeout(r,0));
    }
    st.free();
    const raw = db.export(); db.close();
    // Nettoyer le nom de fichier
    const cleanName = s => (s||'?').replace(/[^a-zA-Z0-9_\-]/g,'_').slice(0,40);
    const fname = `litto2mbtiles_${cleanName(S.ptStart?.name)}_${cleanName(S.ptEnd?.name)}_z${S.zoomMin}-${S.zoomMax}.mbtiles`;
    // Attacher au DOM pour garantir le clic (requis par certains navigateurs)
    ...
    log(`Exporté : ${(raw.byteLength/1048576).toFixed(1)} Mo — ${entries.length} tuiles — ${fname}`, 'success');
  } catch(e) {
