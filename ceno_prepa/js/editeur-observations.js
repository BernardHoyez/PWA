...
const EditObservations = (() => {
  ...
  function _remplirFormulaire(obs) {
    ...
    if (preview && obs.photo) {
      preview.src = '../ceno_util/' + obs.photo;
    ...
    // Audio actuel
    const audioPath = document.getElementById('obs-form-audio-path');
    if (audioPath) audioPath.textContent = obs.audio || 'Aucun fichier audio';
    const audioPreviewBtn = document.getElementById('obs-audio-preview-btn');
    const audioPreviewEl  = document.getElementById('obs-audio-preview-player');
    if (audioPreviewEl) {
      audioPreviewEl.pause();
      audioPreviewEl.src = obs.audio ? '../ceno_util/' + obs.audio : '';
    }
    if (audioPreviewBtn) {
      audioPreviewBtn.style.display = obs.audio ? 'flex' : 'none';
    }
  ...
  function _bindFormEvents() {
    ...
    // Audio upload
    const audioInput      = document.getElementById('obs-form-audio');
    const audioPreviewBtn = document.getElementById('obs-audio-preview-btn');
    const audioPreviewEl  = document.getElementById('obs-audio-preview-player');
    if (audioInput) {
      audioInput.addEventListener('change', () => {
        ...
        if (!file) return;
        const url = URL.createObjectURL(file);
        document.getElementById('obs-form-audio-path').textContent =
          'audio/observations/' + file.name;
        if (audioPreviewEl) { audioPreviewEl.src = url; }
        if (audioPreviewBtn) { audioPreviewBtn.style.display = 'flex'; }
      });
    }

    // Lecture aperçu audio
    if (audioPreviewBtn && audioPreviewEl) {
      audioPreviewBtn.addEventListener('click', () => {
        if (audioPreviewEl.paused) {
          audioPreviewEl.play().catch(e => console.warn('[audio preview]', e));
          audioPreviewBtn.innerHTML = '<span>⏹</span><span>Arrêter</span>';
          audioPreviewBtn.style.background = '#5a2a8a';
        } else {
          audioPreviewEl.pause(); audioPreviewEl.currentTime = 0;
          audioPreviewBtn.innerHTML = '<span>▶</span><span>Écouter l\'aperçu</span>';
          audioPreviewBtn.style.background = '#7b4fa6';
        }
      });
      audioPreviewEl.onended = () => {
        audioPreviewBtn.innerHTML = '<span>▶</span><span>Écouter l\'aperçu</span>';
        audioPreviewBtn.style.background = '#7b4fa6';
      };
    }
  ...
  function _sauvegarder() {
    ...
    // Audio
    const audioPathEl = document.getElementById('obs-form-audio-path');
    if (audioPathEl) {
      const ap = audioPathEl.textContent;
      if (ap && ap !== 'Aucun fichier audio') selected.audio = ap;
    }
