...
const Infos = (() => {
  ...
  /* ── Liste des fiches — sans vignette image ── */
  ...
  function ouvrirPage(pageId) {
    ...
    /* Image en tête de page si définie, avant le titre et le contenu */
    const imageTete = page.image_banniere
      ? `<figure class="info-figure-tete">
           <img src="${page.image_banniere}"
             alt="${page.titre}"
             onerror="this.parentElement.style.display='none'">
         </figure>`
      : '';

    content.innerHTML = imageTete
      + `<h2>${page.titre}</h2>`
      + _renderContenu(page.contenu);
