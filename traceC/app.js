...
function download(f){
  ...
  if ('showSaveFilePicker' in window) {
    ...
    window.showSaveFilePicker(opts).then(fileHandle => {
      ...
      // Si annulé ou erreur, téléchargement classique
    ...
    // Téléchargement classique pour les navigateurs non compatibles
    const a=document.createElement('a');
    a.href=URL.createObjectURL(blob);
    a.download='${filename}.'+ext;
    a.click();
...
function share(){
  ...
  if ('showSaveFilePicker' in window) {
    ...
    window.showSaveFilePicker(opts).then(fileHandle => {
      ...
      // Si annulé, essayer Web Share API
      if (err.name === 'AbortError') {
        return;
      }
      // Fallback sur Web Share ou téléchargement
...
async function exportPDF(){
  ...
  try{
    ...
    // Petite pause supplémentaire pour être sûr
