...
// Extraire waypoints et POI - VERSION SIMPLIFIÉE SANS DESCRIPTIONS
  ...
  if (geojson.type === 'FeatureCollection') {
    geojson.features.forEach(feature => {
      if (feature.geometry && feature.geometry.type === 'Point') {
        // Récupérer uniquement le nom, ignorer les descriptions
        let name = 'Waypoint';
        ...
        // Si le nom est vide, utiliser "Waypoint"
        if (!name || name === '') {
          name = 'Waypoint';
        }
...
async function handleFile(file) {
  ...
  // Générer le code des waypoints - VERSION SIMPLIFIÉE (nom uniquement)
  let waypointsCode = '';
  ...
  if (waypoints.length > 0) {
    ...
    // Stocker uniquement position et nom
