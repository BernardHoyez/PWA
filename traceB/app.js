...
function extractWaypoints(geojson) {
  ...
  if (geojson.type === 'FeatureCollection') {
    geojson.features.forEach(feature => {
      if (feature.geometry && feature.geometry.type === 'Point') {
        // Sécuriser le nom et la description
        let name = 'Point';
        let desc = '';
        ...
        if (feature.properties) {
          ...
          if (feature.properties.desc) {
            desc = String(feature.properties.desc).trim();
          } else if (feature.properties.description) {
            desc = String(feature.properties.description).trim();
          }
        }
        
        // Si le nom est vide (cas OruxMaps), utiliser "Waypoint"
        if (!name || name === '') {
          name = 'Waypoint';
        }
        
        // Supprimer TOUT le HTML de la description (trop complexe à gérer)
        desc = desc
  ...
  console.log('extractWaypoints: trouvés =', waypoints.length);
  return waypoints;
}
...
function preprocessOruxMapsKML(xmlText) {
  // OruxMaps crée 2 dossiers: "Tracks" (avec gx:Track) et "Paths" (avec LineString)
  // On supprime le dossier "Tracks" car gx:Track n'est pas supporté par toGeoJSON
  // et on garde seulement "Paths" qui contient des LineString standards
  
  const parser = new DOMParser();
  const doc = parser.parseFromString(xmlText, 'text/xml');
  
  // Trouver et supprimer le dossier "Tracks"
  const folders = doc.getElementsByTagName('Folder');
  for (let i = folders.length - 1; i >= 0; i--) {
    const folder = folders[i];
    const nameElement = folder.getElementsByTagName('name')[0];
    if (nameElement && nameElement.textContent.trim() === 'Tracks') {
      console.log('Suppression du dossier Tracks (gx:Track non supporté)');
...
async function handleFile(file) {
  ...
  // Vérifier que le GeoJSON est valide
  console.log('GeoJSON généré:', geojson);
  
  if (!geojson || !geojson.features || geojson.features.length === 0) {
    document.getElementById('msg').textContent = '';
    return alert('Aucune géométrie trouvée dans le fichier. Le fichier semble vide ou mal formaté.');
  }
  ...
  console.log('Waypoints extraits:', waypoints.length, waypoints);
  ...
  try {
    ...
    console.error('Erreur calcul bbox:', error);
    document.getElementById('msg').textContent = '';
    return alert('Erreur lors du calcul des limites de la trace: ' + error.message);
  }
  ...
  // Générer le code des waypoints - MÉTHODE SÉCURISÉE
  let waypointsCode = '';
  let waypointsDataArray = [];
  
  if (waypoints.length > 0) {
    console.log('Génération du code pour', waypoints.length, 'waypoints');
    
    // Stocker les waypoints dans un tableau sérialisé de manière sûre
    waypoints.forEach((wp, idx) => {
      waypointsDataArray.push({
        lat: wp.coords[1],
        lon: wp.coords[0],
        name: wp.name,
        desc: wp.desc
    ...
    const waypointsDataString = JSON.stringify(waypointsDataArray)
      .replace(/</g, '\\u003c')
      .replace(/>/g, '\\u003e')
      .replace(/&/g, '\\u0026')
      .replace(/'/g, "\\'");
    ...
    waypointsCode = `
    const waypointsData = ${waypointsDataString};
    waypointsData.forEach(function(wp) {
      const popupContent = wp.desc ? '<b>' + wp.name + '</b><br>' + wp.desc : '<b>' + wp.name + '</b>';
      L.marker([wp.lat, wp.lon])
        .bindPopup(popupContent)
        .addTo(map);
    });`;
    
    console.log('Code waypoints généré pour', waypointsDataArray.length, 'waypoints');
  } else {
  ...
  if (photos.length > 0) {
    const photosString = JSON.stringify(photos)
      ...
      .replace(/&/g, '\\u0026')
      .replace(/'/g, "\\'");
    photosData = `const photos = ${photosString};`;
    photosCode = `
  ...
  // Sérialiser le GeoJSON de manière sûre pour éviter les problèmes d'échappement
...
if (!g || !g.features || g.features.length === 0) {
  document.body.innerHTML = '<div style="padding:20px;text-align:center"><h2>Erreur</h2><p>Aucune donnée géographique trouvée</p></div>';
  throw new Error('GeoJSON vide');
}

console.log('GeoJSON chargé:', g);
...
try {
  ...
  document.body.innerHTML = '<div style="padding:20px;text-align:center"><h2>Erreur</h2><p>Impossible d initialiser la carte: ' + error.message + '</p></div>';
}
