  ...
  if (geojson.type === 'FeatureCollection') {
    geojson.features.forEach(feature => {
      if (feature.geometry && feature.geometry.type === 'Point') {
        ...
        // Nettoyer la description des balises HTML pour éviter les problèmes
...
async function handleFile(file) {
  ...
  if (waypoints.length > 0) {
    ...
    waypointsCode = waypoints.map((wp, idx) => {
      console.log('Waypoint', idx, ':', wp);
      // Échapper les guillemets et caractères spéciaux dans le nom et description
      const safeName = (wp.name || 'Waypoint')
        .replace(/\\/g, '\\\\')
        .replace(/'/g, "\\'")
        .replace(/"/g, '\\"')
        .replace(/\n/g, ' ')
      ...
      const popupContent = safeDesc ? `<b>${safeName}</b><br>${safeDesc}` : `<b>${safeName}</b>`;
      
      return `L.marker([${wp.coords[1]}, ${wp.coords[0]}])
        .bindPopup('${popupContent}')
        .addTo(map);`;
    }).join('\n');
    console.log('Code waypoints généré:', waypointsCode);
  } else {
