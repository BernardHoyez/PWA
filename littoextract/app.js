...
const CONFIG = {
  ...
  // URL TMS BD ORTHO IGN Géoplateforme (accès libre sans clé)
  // Deux URLs possibles — on teste la principale, fallback sur l'autre
  IGN_TMS:      'https://data.geopf.fr/tms/1.0.0/ORTHOIMAGERY.ORTHOPHOTOS/{z}/{x}/{y}.jpeg',
  IGN_TMS_ALT:  'https://wxs.ign.fr/essentiels/geoportail/tms/1.0.0/ORTHOIMAGERY.ORTHOPHOTOS/{z}/{x}/{y}.jpeg',
  RETRY_MAX:    3,
  RETRY_DELAY:  800,          // ms
...
function getTilesForPolygon(polygon, zMin, zMax) {
  ...
  const polyBbox = { minX: bbox[0], minY: bbox[1], maxX: bbox[2], maxY: bbox[3] };

  for (let z = zMin; z <= zMax; z++) {
    const [xMin, yMax2] = lngLatToTile(bbox[0], bbox[3], z); // coin NW
    const [xMax, yMin2] = lngLatToTile(bbox[2], bbox[1], z); // coin SE

    for (let x = xMin; x <= xMax; x++) {
      for (let y = yMin2; y <= yMax2; y++) {
        // Test d'overlap bbox — fiable quelle que soit la version de Turf
        const [tW, tS, tE, tN] = tileToBbox(x, y, z);
        const tileBbox = { minX: tW, minY: tS, maxX: tE, maxY: tN };
        if (bboxOverlaps(polyBbox, tileBbox)) {
          tiles.push([z, x, y]);
        }
      }
    }
...
// Test d'overlap de deux bounding boxes
...
async function downloadTiles() {
  ...
  // ── Sonde : vérifier quelle URL IGN répond ──────────────────
  log('Test de l\'URL IGN en cours…');
  const [tz, tx, ty] = state.tiles[Math.floor(state.tiles.length / 2)]; // tuile médiane
  const urlMain = CONFIG.IGN_TMS.replace('{z}', tz).replace('{x}', tx).replace('{y}', ty);
  const urlAlt  = CONFIG.IGN_TMS_ALT.replace('{z}', tz).replace('{x}', tx).replace('{y}', ty);

  let activeUrl = CONFIG.IGN_TMS;
  try {
    const probe = await fetch(urlMain, { method: 'HEAD', cache: 'no-store' });
    if (!probe.ok && probe.status !== 404) throw new Error(`HTTP ${probe.status}`);
    log('URL principale Géoplateforme OK', 'success');
  } catch (e) {
    log(`URL principale inaccessible (${e.message}), bascule sur URL alternative…`, 'warn');
    try {
      const probe2 = await fetch(urlAlt, { method: 'HEAD', cache: 'no-store' });
      if (probe2.ok || probe2.status === 404) {
        activeUrl = CONFIG.IGN_TMS_ALT;
        log('URL alternative OK', 'success');
      }
    } catch (e2) {
      log(`Les deux URLs IGN sont inaccessibles. Vérifiez votre connexion.`, 'error');
      updateStatusBadge('error', 'IGN inaccessible');
      return;
    }
  }
  ...
  updateStatusBadge('', `Téléchargement — 0 / ${total}`);
  ...
  async function worker() {
    while (queue.length > 0 && !state.abortCtrl.signal.aborted) {
      ...
      if (done % 50 === 0) updateStatusBadge('', `${pct}% — ${state.tileMap.size} tuiles`);
    }
  }
...
async function exportMBTiles() {
  ...
  try {
    ...
    if (!state.SQL) {
      ...
      // sql.js chargé via <script> expose window.initSqlJs
      const initFn = window.initSqlJs || (typeof initSqlJs !== 'undefined' ? initSqlJs : null);
      if (!initFn) throw new Error('sql.js non chargé — vérifiez la connexion CDN');
      state.SQL = await initFn({
        locateFile: f => `https://cdnjs.cloudflare.com/ajax/libs/sql.js/1.10.3/${f}`
      });
      log('sql.js initialisé', 'success');
    }
...
function updateStatusBadge(badgeState, msg) {
  const dot = document.querySelector('.status-dot');
  const txt = document.querySelector('.status-text');
  if (dot) dot.className = `status-dot ${badgeState}`;
  if (txt) txt.textContent = msg;
}

// ── Init ──────────────────────────────────────────────────────
async function init() {
  updateStatusBadge('', 'Initialisation…');
  ...
  if (ok) {
    ...
    computeWaypoints();
    initPreviewMap();
    updateStatusBadge('ready', `${state.tileCount} tuiles calculées`);
  } else {
    updateStatusBadge('error', 'Erreur chargement données');
  }
