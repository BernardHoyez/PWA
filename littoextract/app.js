...
async function fetchTileWithRetry(z, x, y, signal) {
  // IGN Géoplateforme TMS : Y inversé par rapport à XYZ standard
  const yTMS = (1 << z) - 1 - y;
  ...
  for (let attempt = 0; attempt < CONFIG.RETRY_MAX; attempt++) {
    ...
    try {
      ...
      if (attempt === 0 && z === CONFIG.ZOOM_MIN) {
        log(`Erreur fetch Z${z}/${x}/${yTMS} : ${e.message}`, 'error');
      }
...
async function downloadTiles() {
  ...
  const tyTMS = (1 << tz) - 1 - ty;
  const testUrl = CONFIG.IGN_TMS.replace('{z}', tz).replace('{x}', tx).replace('{y}', tyTMS);
