...
async function fetchTileWithRetry(z, x, y, signal) {
  ...
  for (let attempt = 0; attempt < CONFIG.RETRY_MAX; attempt++) {
    ...
    try {
      ...
      if (r.status === 404) return null;  // tuile absente normalement

      if (r.status === 429 || r.status === 503) {
        // Rate-limit : attendre plus longtemps
        const wait = CONFIG.RETRY_BASE_MS * Math.pow(2, attempt);
        log(`Rate-limit Z${z}/${x}/${y} — attente ${wait}ms`, 'warn');
...
async function downloadTiles() {
  ...
  async function worker() {
    while (queue.length > 0 && !S.abortCtrl.signal.aborted) {
      ...
      const data = await fetchTileWithRetry(z, x, y, S.abortCtrl.signal);
      if (S.abortCtrl.signal.aborted) return;

      if (data === null) {
        notfound++;
      } else {
        S.tileMap.set(`${z}/${x}/${y}`, data);
      }
...
function prepareViewerProtocol() {
  ...
  // Enregistre un protocole "litto://" géré entièrement en JS
  maplibregl.addProtocol('litto', (params, callback) => {
    // URL format : litto://tiles/{z}/{x}/{y}
    ...
    if (!data) {
      callback(new Error('Tile not found'), null, null, null);
      return { cancel: () => {} };
    }

    callback(null, data.buffer, null, null);
    return { cancel: () => {} };
  });

  log('Protocole visualiseur enregistre', 'success');

  // Si le visualiseur est déjà ouvert, recharger les tuiles
