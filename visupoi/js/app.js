...
document.addEventListener('DOMContentLoaded', () => {
  ...
  input.addEventListener('change', async (e) => {
    ...
    try {
      ...
      // stockage global utile pour debug / access
      window.visupoiData = { visit, mediaMap };

      // initialise la carte si besoin et affiche les POI
      initMap();
      renderPOIs(visit, mediaMap);

      // démarre la géolocalisation (watchPosition)
      startGeolocation();
    } catch (err) {
  ...
  // support basique pour drag/drop (optionnel)
