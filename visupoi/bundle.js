(function () {
  ...
  const PopupContent = ({ pois, mediaData }) => {
    ...
    }, [selectedPoi]); // Re-run if selectedPoi changes to re-apply to new DOM
  ...
  const MapDisplay = ({ visitData, mediaData, onReset }) => {
    ...
    React.useEffect(() => {
      ...
      groupedPois.forEach((poisAtLocation) => {
        ...
        marker.on('click', () => {
          ...
          popupRoot.render(e(PopupContent, { pois: poisAtLocation, mediaData: mediaData }));
          marker.bindPopup(popupNode, { minWidth: 250 }).openPopup();
          
          // Ensure unmounting when popup is closed
    ...
    return e("div", { className: "relative" },
      e("div", { id: "map", ref: mapContainerRef, className: "h-screen w-screen z-0" }),
