...
// --- API IGN Altimétrie (sans clé) ---

async function getIGNAltitude(lat, lon) {
  const url = `https://data.geopf.fr/altimetrie/1.0/calcul/elevation.json?lat=${lat}&lon=${lon}`;
  const response = await fetch(url);
  const data = await response.json();
  return data.elevation;
}

async function correctAltitudesWithIGN(points, onProgress) {
  const corrected = [];
  ...
  for (let i = 0; i < points.length; i++) {
    ...
    let ele = null;
    try {
      ele = await getIGNAltitude(p.lat, p.lon);
    } catch (e) {
      ele = p.ele; // fallback
    }
    ...
    // éviter de saturer l’API (~7 requêtes/s)
...
async function handleTrackGeoJSON(geojson) {
  ...
  // --- Correction automatique via IGN ---
  document.getElementById('dplus').textContent = "Correction IGN...";
  document.getElementById('ibp').textContent = "Correction IGN...";
  document.getElementById('difficulty').textContent = "Correction IGN...";

  points = await correctAltitudesWithIGN(points);

  // --- Recalcul des segments ---
...
// --- Lissage altimétrique modéré ---

function medianFilter(values, windowSize = 5) {
...
function computeDplusSmoothed(points) {
  ...
  const median = medianFilter(raw, 5);
  const smooth = movingAverage(median, 3);
...
// --- IBP calibré pour viser ~55 ---

function estimateIBP(distanceKm, dplus) {
  return 0.05 * dplus + 2.3 * distanceKm;
}
