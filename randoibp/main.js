...
function medianFilter(values, windowSize = 11) {
...
function computeDplusSmoothed(points) {
  ...
  const median = medianFilter(raw, 11);
  const smooth = movingAverage(median, 9);

  let dplus = 0;
  for (let i = 1; i < smooth.length; i++) {
    const dz = smooth[i] - smooth[i - 1];
    if (dz > 2) dplus += dz; // seuil 2 m pour éliminer le bruit
  }
  return dplus;
}
...
function estimateIBP(distanceKm, dplus) {
  return 0.06 * dplus + 2.0 * distanceKm;
}
