...
closeMapBtn?.addEventListener('click', () => {
  ...
  // Détruire la carte proprement pour éviter les bugs et permettre une nouvelle création
  if (map) {
    map.remove();
    map = null;
    mapMarker = null;
...
function initMap() {
  if (!map) {
    map = L.map('map', { zoomControl: true }).setView([48.858, 2.294], 5);
    L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', {
      attribution: '© OSM contributors'
    }).addTo(map);

    // Si latitude et longitude déjà remplies, centrer dessus et placer le marqueur
    if (poiLat.value && poiLon.value) {
      const latlng = [parseFloat(poiLat.value), parseFloat(poiLon.value)];
      map.setView(latlng, 15);
      if (mapMarker) map.removeLayer(mapMarker);
      mapMarker = L.marker(latlng, { draggable: false }).addTo(map);
    }

    map.on('click', function(e) {
      if (mapMarker) map.removeLayer(mapMarker);
      mapMarker = L.marker(e.latlng, { draggable: false }).addTo(map);
      poiLat.value = e.latlng.lat.toFixed(6);
      poiLon.value = e.latlng.lng.toFixed(6);
    });
  }
}
