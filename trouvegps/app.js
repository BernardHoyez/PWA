...
function showMap(lat, lon) {
  if (window.map) {
    ...
    window.map = L.map('map', {zoomControl:true}).setView([lat, lon], 15);
...
// Main handler - waits for EXIF availability if necessary
function initPhotoHandler() {
  const input = document.getElementById('photoInput');
  if (typeof EXIF === 'undefined') {
    setStatus('ERREUR: EXIF non défini. Tentative de rechargement dynamique...');
    // Try to dynamically load exif.min.js then re-init
    var s = document.createElement('script');
    s.src = 'exif.min.js';
    s.onload = function(){ setStatus('EXIF chargé dynamiquement. Prêt.'); bindHandler(); };
    s.onerror = function(){ setStatus('Échec chargement exif.min.js — vérifier présence du fichier.'); };
    document.body.appendChild(s);
  } else {
    setStatus('EXIF OK. Prêt.');
    bindHandler();
  }
}
  ...
  input.addEventListener('change', function(e){
    ...
    try {
      EXIF.getData(file, function(){
        ...
        setStatus('Point affiché : ' + latitude.toFixed(6) + ', ' + longitude.toFixed(6));
      });
    } catch(err) {
      console.error(err);
      setStatus('Erreur lors de la lecture EXIF : ' + err.message);
    }
  });
}
