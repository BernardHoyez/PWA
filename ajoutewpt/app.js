...
async function loadGPX(file) {
  ...
  doc.querySelectorAll('trk trkseg').forEach(seg => {
    ...
    if (pts.length > 1) layers.push(L.polyline(pts, { color: '#5db87a', weight: 3, opacity: 0.9 }).addTo(state.map));
  });
  doc.querySelectorAll('rte').forEach(rte => {
    const pts = [...rte.querySelectorAll('rtept')]
      .map(p => [+p.getAttribute('lat'), +p.getAttribute('lon')])
      .filter(([a, b]) => !isNaN(a) && !isNaN(b));
    if (pts.length > 1) layers.push(L.polyline(pts, { color: '#e8a838', weight: 3, opacity: 0.9 }).addTo(state.map));
  });
...
document.getElementById('saveLocBtn').addEventListener('click', async () => {
  ...
  for (const wpt of wpts) {
    ...
    // Description HTML : image + commentaire
    const descHtml = [
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    <name>${escXml(folderName)}_loc</name>
    <description>Waypoints géolocalisés — AjouteWPT</description>
${placemarks}
  </Document>
</kml>`;
  ...
  // Aussi inclure un JSON de référence avec toutes les données
  const jsonData = {
  ...
  showToast(`💾 "${folderName}_loc.kmz" — ${wpts.length} waypoint(s) exporté(s)`);
});
