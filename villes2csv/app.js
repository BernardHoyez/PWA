...
// ─── Save CSV ────────────────────────────────────────────────
// ─── RFC 4180 helper ──────────────────────────────────────────
// Entoure toujours la valeur de guillemets doubles (guillemets internes doublés).
...
function onSave() {
  ...
  // En-tête sans guillemets (noms simples sans virgule)
  const header = 'nom,initiale,latitude,longitude,affichage_nominatim,label';
  ...
  const rows = toSave.map(c => {
    ...
    return [
      csvQuote(c.nom),
      csvQuote(initiale),
      c.latitude,       // numérique : pas de guillemets
      c.longitude,
      csvQuote(c.affichage_nominatim || ''),
      csvQuote(c.nom)   // colonne label = nom, pour étiquetage QGIS/SIG
    ].join(',');
  });

  // CRLF conforme RFC 4180 + BOM UTF-8 (reconnaissance QGIS/Excel/LibreOffice)
