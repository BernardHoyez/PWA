    ...
    createTile: function (coords, done) {
      ...
      getTileFromCache(z, x, y).then((url) => {
        if (url) {
          ...
          // Si la tuile n'est pas disponible hors ligne, utiliser la source en ligne
          const onlineUrl = `https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2&STYLE=normal&TILEMATRIXSET=PM&TILEMATRIX=${z}&TILEROW=${y}&TILECOL=${x}&FORMAT=image/png`;
          tile.src = onlineUrl;
          done(null, tile);
        }
      }).catch(() => {
...
// Fonction pour basculer entre les couches en ligne et hors ligne
function setupOfflineMode() {
  const offlineLayer = createOfflineTileLayer();

  if (isOnline()) {
    // Utiliser la couche en ligne par défaut
    ...
    // Utiliser la couche hors ligne
    planLayer = new offlineLayer('https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2&STYLE=normal&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}&FORMAT=image/png', {
...
async function generateMBTilesFile() {
  const dbName = 'matrace2MBTilesDB';
  const storeName = 'tiles';
