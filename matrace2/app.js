...
// Fonction pour vérifier si une tuile est disponible hors ligne
async function getTileFromCache(z, x, y) {
    ...
    request.onsuccess = (event) => {
      ...
      getRequest.onsuccess = () => {
        if (getRequest.result) {
          const blob = new Blob([getRequest.result.blob], { type: 'image/png' });
...
// Fonction pour créer une couche Leaflet personnalisée qui utilise les tuiles hors ligne
function createOfflineTileLayer() {
  return L.TileLayer.extend({
    createTile: function (coords, done) {
      ...
      getTileFromCache(z, x, y).then((url) => {
        if (url) {
          tile.src = url;
          done(null, tile);
        } else {
          // Si la tuile n'est pas disponible hors ligne, afficher une tuile vide
...
function setOfflineMode() {
  isOfflineMode = true;
  map.removeLayer(planLayer);
  planLayer = new createOfflineTileLayer('https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2&STYLE=normal&TILEMATRIXSET=PM&TILEMATRIX={z}&TILEROW={y}&TILECOL={x}&FORMAT=image/png', {
...
// Fonction pour enregistrer un fichier .mbtiles dans un dossier sélectionné
async function saveMBTilesFile(blob, dirHandle, fileName) {
  try {
    ...
    alert(`Fichier ${fileName} enregistré avec succès dans le dossier sélectionné.`);
  } catch (error) {
...
// Fonction pour générer et enregistrer le fichier MBTiles
async function generateAndSaveMBTilesFile() {
  const dbName = 'matrace2MBTilesDB';
  const storeName = 'tiles';
...
async function createMBTiles(tiles) {
  ...
  const metadata = [
    { name: 'name', value: 'matrace2' },
    { name: 'format', value: 'png' },
    { name: 'version', value: '1' },
    { name: 'type', value: 'baselayer' },
    { name: 'description', value: 'Tuiles IGN pour matrace2' }
  ];

  metadata.forEach(m => {
    db.run(`INSERT INTO metadata (name, value) VALUES (?, ?)`, [m.name, m.value]);
  });
  ...
  for (const tile of tiles) {
    if (tile.blob) {
      ...
      await new Promise((resolve, reject) => {
        ...
        reader.readAsArrayBuffer(tile.blob);
      });
    }
  }

  const mbtilesData = db.export();
  return new Blob([mbtilesData], { type: 'application/x-sqlite3' });
}
...
function initMap() {
  ...
  // Par défaut, on commence en mode en ligne
  setOnlineMode();
...
async function downloadTiles(lat, lon, radiusKm) {
  ...
  for (const tile of tiles) {
    const url = `https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2&STYLE=normal&TILEMATRIXSET=PM&TILEMATRIX=${tile.z}&TILEROW=${tile.y}&TILECOL=${tile.x}&FORMAT=image/png`;
