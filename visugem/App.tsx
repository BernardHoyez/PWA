...
function App() {
  const { processedData, isLoading, error, processZipFile, cleanup: cleanupZipData } = useZipProcessor();
  const [userPosition, setUserPosition] = useState<UserPosition | null>(null);
  const [geoError, setGeoError] = useState<string | null>(null);
  const watchIdRef = useRef<number | null>(null);

  // Gère le cycle de vie du suivi GPS en fonction de la présence des données
  useEffect(() => {
    if (processedData) {
      // Démarre le suivi si les données sont chargées et qu'il n'est pas déjà actif
      ...
      // Arrête le suivi si les données sont effacées et qu'un suivi est actif
      if (watchIdRef.current !== null) {
    ...
    // Nettoyage final lorsque le composant App est démonté
    return () => {
      if (watchIdRef.current !== null) {
        navigator.geolocation.clearWatch(watchIdRef.current);
      }
  ...
  const handleNewFileClick = () => {
    // Stoppe explicitement le suivi GPS et réinitialise l'état pour un retour immédiat
    if (watchIdRef.current !== null) {
      navigator.geolocation.clearWatch(watchIdRef.current);
      watchIdRef.current = null;
    }
    ...
    // Nettoie les données du ZIP (ce qui déclenche aussi l'useEffect ci-dessus et démonte la carte)
    cleanupZipData();
  };
