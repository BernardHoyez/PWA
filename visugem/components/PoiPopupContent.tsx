...
function haversineDistance(pos1: UserPosition, pos2: { lat: number, lng: number }): number {
    if (!pos1) return 0;
...
function calculateAzimuth(pos1: UserPosition, pos2: { lat: number, lng: number }): number {
    if (!pos1) return 0;
    ...
    return (θ * 180 / Math.PI + 360) % 360; // Azimut en degrés
}
// --- Fin des fonctions GPS ---


const SinglePoiDisplay = ({ poi, media, userPosition }: { poi: Poi, media: MediaData[number], userPosition: UserPosition }) => {
    const [poiLat, poiLng] = poi.location.split(',').map(Number);
    const distance = userPosition ? haversineDistance(userPosition, { lat: poiLat, lng: poiLng }) : null;
    const azimuth = userPosition ? calculateAzimuth(userPosition, { lat: poiLat, lng: poiLng }) : null;
