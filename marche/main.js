...
// + sélection du point de départ par clic et robustification de la saisie manuelle
...
// --- tuiles: téléchargement (inchangé) ---
...
let selectedStartMarker = null; // marqueur temporaire placé par clic
let tracking = false;
let watchId = null;
let trackCoords = [];
let waypoints = [];
...
function initMap(){
  ...
  // Clic sur la carte : remplir les champs lat/lon et placer un marqueur de sélection
  map.on('click', function(e) {
    const lat = e.latlng.lat;
    const lon = e.latlng.lng;
    // remplir champs (6 décimales)
    const latInput = document.getElementById('lat');
    const lonInput = document.getElementById('lon');
    latInput.value = lat.toFixed(6);
    lonInput.value = lon.toFixed(6);

    // placer ou déplacer le marqueur de sélection
    if (selectedStartMarker) {
      selectedStartMarker.setLatLng([lat, lon]);
    } else {
      selectedStartMarker = L.marker([lat, lon], {opacity:0.9}).addTo(map).bindPopup('Point sélectionné (cliquer "Enregistrer point de départ")').openPopup();
    }
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  btnSave.addEventListener('click', () => {
    // robustifier la lecture : accepter virgule ou point, trim
    const rawLat = latInput.value.trim().replace(',', '.');
    const rawLon = lonInput.value.trim().replace(',', '.');
    const lat = parseFloat(rawLat);
    const lon = parseFloat(rawLon);
    if (Number.isNaN(lat) || Number.isNaN(lon)) return alert('Coordonnées invalides. Utilisez le format degrés décimaux (ex: 46.123456, 2.123456)');
    if (lat < -90 || lat > 90 || lon < -180 || lon > 180) return alert('Coordonnées hors plage valide (lat -90→90, lon -180→180)');
    saveStartPoint(lat, lon);
    // retirer marqueur de sélection si présent
    if (selectedStartMarker) {
      selectedStartMarker.remove();
