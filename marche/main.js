// main.js — fichier complet prêt à remplacer
// Fonctionnalités : carte IGN, sélection départ, estimation et calcul exact des tuiles,
// plafond automatique (>10000 tuiles demande confirmation explicite), téléchargement (zmax ≤ 16),
// sauvegarde optionnelle via File System Access (session folder), tracking GPS, photo géolocalisée,
// enregistrement audio, export GPX/KML, progression visuelle, installation PWA prompt.
...
async function chooseRootDir(){
  if (!window.showDirectoryPicker) throw new Error('File System Access non supportée par votre navigateur');
...
// --- tuiles: conversion indices / bbox / distance ---
...
// --- estimation rapide (approx) pour réponse instantanée ---
...
// --- version asynchrone non-bloquante pour calcul exact (avec callback de progression) ---
async function listTilesAsync(lat, lon, radiusMeters = 10000, zmin = 13, zmax = 16, progressCb = ()=>{}){
  ...
  const tiles = [];
  // compute rough total area to help progress UI (upper bound)
  let totalEstimate = 0;
  for (let z = zmin; z <= zmax; z++){
  ...
  let countSoFar = 0;
  const yieldEvery = 1500; // iterations before yielding
  let iter = 0;
    ...
    for (let x = xmin; x <= xmax; x++){
      for (let y = ymin; y <= ymax; y++){
        ...
        if ((iter & (yieldEvery - 1)) === 0){
          progressCb({countSoFar, totalEstimate, z, x, y});
          await new Promise(r => setTimeout(r, 0)); // yield to UI
        }
      }
    }
    progressCb({countSoFar, totalEstimate, z, x:null, y:null});
  }
  progressCb({countSoFar: tiles.length, totalEstimate, z:null, done:true});
  return tiles;
}

// --- téléchargement des tuiles avec progression et écriture en session (si présent) ---
...
// cache de dernier calcul pour éviter recalcul si inchangé
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (btnChooseFolder) btnChooseFolder.addEventListener('click', async () => {
    try {
      await chooseRootDir();
  ...
  // compute estimate (instant + exact async)
  if (btnCompute) btnCompute.addEventListener('click', async () => {
    ...
    // exact calculation async, update UI progressively
  ...
  // download tiles: will perform exact count if not computed or if inputs changed
  if (btnDownload) btnDownload.addEventListener('click', async () => {
    ...
    // if lastTilesCache matches current parameters, reuse; else compute exact list
    let tiles = null;
    if (lastTilesCache && lastTilesCache.lat === lat && lastTilesCache.lon === lon && lastTilesCache.zmin === zmin && lastTilesCache.zmax === zmax && lastTilesCache.tiles){
      ...
      // compute exact list (with progress UI)
      const tileEstimateDiv = document.getElementById('tile-estimate');
    ...
    // automatic ceiling: if count > 10000 require explicit confirmation (typed)
    ...
    try {
      ...
      await downloadTilesAroundFromList(tiles, (document.getElementById('tile-url') || {}).value || '', (p) => {
  ...
  // Controls for start/photo/audio/stop wiring (buttons exist in index.html)
...
// Stop tracking and export GPX/KML (write to session if present)
...
// Photo capture (camera or fallback input), overlay coordinates, save to session/media or download
...
// Audio recording: start/stop handled externally; onstop saves file to session/media or downloads it
