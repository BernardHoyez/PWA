...
const EditFossiles = (() => {
  ...
  function init(fossilesData) {
    data = fossilesData || [];
    _renderGroupes();
    _peuplerSelectGroupes();  /* peupler le select maintenant que data est chargé */
    _showEmpty();
  }

  /* ── Peupler le select des groupes ── */
  function _peuplerSelectGroupes() {
    const sel = document.getElementById('fos-form-groupe');
  ...
  function _bindFormEvents() {
    ...
    /* select groupes peuplé dans init() via _peuplerSelectGroupes() */
