class FieldGuideApp {
                ...
                try {
                    ...
                    if (mediaFile) {
                        ...
                        if (media.key === 'image') {
                            ...
                            console.log(`Image URL créée:`, poi[media.prop], 'type:', mimeType, 'taille:', imageBlob.size);
                            
                            // Créer aussi une version base64 comme fallback
                            ...
                            console.log(`Image base64 créée, taille:`, base64String.length, 'caractères');
                            
                        } else {
                            // Pour vidéo et audio, utiliser la méthode normale
    ...
    showPOIPopup(poi) {
        ...
        // Test diagnostic complet pour les images
        let mediaContent = '';
        
        if (poi.imageUrl || poi.image) {
            console.log('=== DIAGNOSTIC IMAGE ===');
            console.log('poi.imageUrl:', poi.imageUrl);
            console.log('poi.imageUrl_base64 existe:', !!poi.imageUrl_base64);
            console.log('poi.image (nom fichier):', poi.image);
            
            mediaContent += `
                <div style="margin-bottom: 1rem; border: 2px solid #e0e0e0; padding: 1rem; border-radius: 8px;">
                    <h4 style="margin: 0 0 0.5rem 0; color: #555; font-size: 0.9rem;">📸 Diagnostic Image</h4>
                    
                    <div style="background: #f0f0f0; padding: 0.5rem; border-radius: 4px; margin-bottom: 0.5rem; font-size: 0.8rem; font-family: monospace;">
                        <div><strong>Fichier:</strong> ${poi.image || 'Non défini'}</div>
                        <div><strong>Blob URL:</strong> ${poi.imageUrl ? 'Créée' : 'Absente'}</div>
                        <div><strong>Base64:</strong> ${poi.imageUrl_base64 ? 'Créée (' + Math.round(poi.imageUrl_base64.length/1000) + 'k chars)' : 'Absente'}</div>
                        ${poi.imageUrl ? `<div><strong>URL:</strong> ${poi.imageUrl.substring(0, 50)}...</div>` : ''}
                    </div>
            `;
            
            // Test 1: Image avec blob URL
            if (poi.imageUrl) {
                mediaContent += `
                    <div style="margin-bottom: 0.5rem;">
                        <strong>Test 1: Blob URL</strong><br>
                        <img id="test1-${poi.id}" 
                             src="${poi.imageUrl}" 
                             alt="Test blob" 
                             style="max-width: 200px; height: auto; border: 1px solid #ccc;"
                             onload="
                                console.log('✓ Test 1 SUCCÈS: Blob URL fonctionne');
                                this.nextElementSibling.innerHTML = '✅ Blob URL OK';
                                this.nextElementSibling.style.color = 'green';
                             "
                             onerror="
                                console.error('✗ Test 1 ÉCHEC: Blob URL ne fonctionne pas');
                                this.nextElementSibling.innerHTML = '❌ Blob URL ÉCHEC';
                                this.nextElementSibling.style.color = 'red';
                                this.style.display = 'none';
                             ">
                        <div style="font-size: 0.8rem; margin-top: 0.25rem;">⏳ Test en cours...</div>
                    </div>
                `;
            }
            
            // Test 2: Image avec base64
            if (poi.imageUrl_base64) {
                mediaContent += `
                    <div style="margin-bottom: 0.5rem;">
                        <strong>Test 2: Base64</strong><br>
                        <img id="test2-${poi.id}" 
                             src="${poi.imageUrl_base64}" 
                             alt="Test base64" 
                             style="max-width: 200px; height: auto; border: 1px solid #ccc;"
                             onload="
                                console.log('✓ Test 2 SUCCÈS: Base64 fonctionne');
                                this.nextElementSibling.innerHTML = '✅ Base64 OK';
                                this.nextElementSibling.style.color = 'green';
                             "
                             onerror="
                                console.error('✗ Test 2 ÉCHEC: Base64 ne fonctionne pas');
            ...
            mediaContent += `
                <div style="margin-bottom: 0.5rem;">
                    <strong>Test 3: Image de référence (externe)</strong><br>
                    ...
                    <div style="font-size: 0.8rem; color: green;">✅ Si vous voyez un carré bleu "TEST", les images fonctionnent</div>
                </div>
            `;
            ...
            mediaContent += `
                <div style="margin-top: 1rem;">
                    <button onclick="
                        ...
                        console.log('Blob URL valide?', '${poi.imageUrl}' !== 'undefined');
                        console.log('Base64 valide?', '${poi.imageUrl_base64 ? 'true' : 'false'}');
                        ${poi.imageUrl ? `
                        const testImg = new Image();
                        testImg.onload = () => console.log('✓ Test manuel blob: IMAGE OK');
                        testImg.onerror = () => console.log('✗ Test manuel blob: IMAGE KO');
                        testImg.src = '${poi.imageUrl}';
                        ` : ''}
                        alert('Tests lancés - Regardez la console');
                    " style="
                        background: #2563eb;
                        color: white;
                        border: none;
