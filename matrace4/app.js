...
async function downloadTiles(lat, lon, radiusKm) {
  ...
  request.onsuccess = async (event) => {
    ...
    for (const tile of tiles) {
      // Utiliser l'URL correcte pour les tuiles IGN
      const url = `https://data.geopf.fr/wmts?SERVICE=WMTS&REQUEST=GetTile&VERSION=1.0.0&LAYER=GEOGRAPHICALGRIDSYSTEMS.PLANIGNV2&STYLE=normal&TILEMATRIXSET=PM&TILEMATRIX=${tile.z}&TILEROW=${tile.y}&TILECOL=${tile.x}&FORMAT=image%2Fpng`;

      try {
        const response = await fetch(url, {
          headers: {
            'Referer': 'https://geoportail.gouv.fr'
          }
        });
